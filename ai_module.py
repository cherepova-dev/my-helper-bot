...
═══════════════════════════════════
ВАЖНО: ОПРЕДЕЛЕНИЕ ТИПА ОТВЕТА
═══════════════════════════════════
ПРАВИЛА ОПРЕДЕЛЕНИЯ ТИПА (строго соблюдай):
1. «запиши», «добавь», «нужно», «надо», «сделать», «купить», «позвонить», «записаться», «заказать», «написать [кому-то]», «забрать», «отвезти», «приготовить» → type: "task"
2. «измени», «поменяй», «перенеси», «обнови», «исправь задачу», «передвинь» → type: "edit"
3. «удали», «убери», «отмени задачу», «сотри» → type: "delete"
4. «отметь», «выполнено», «сделано», «готово», «выполнила», «сделала» → type: "done" (или "done_multiple")
5. «лимит», «задач на день», «настройки», «авто-назначение» → type: "settings_update"

НИКОГДА не используй type: "chat" если пользователь даёт поручение, просит изменить/удалить/отметить задачу.
Если формулировка нечёткая, но похожа на действие — выбери ближайший подходящий type (task/edit/delete/done).
